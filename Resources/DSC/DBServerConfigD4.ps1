...
#>
		...
		Script Configure-StoragePool
        	{
                ...
                $DataDisks = $PhysicalDisks | Where-Object FriendlyName -in('PhysicalDisk2')
                $LogandSystemDisks = $PhysicalDisks | Where-Object FriendlyName -in('PhysicalDisk3')
		        ...
		        {
           		    Write-Output "$(get-date) : The data disk is not available. Please review to confirm disks were created successfully. Details of disks are below"
           		    Write-Error($DataDisks) -ErrorAction Stop
           		}
	
                If($LogandSystemDisks)
			    {
			        $totalstorageconfigresult2 = New-StoragePool -FriendlyName "SQLLogsandSystemPool01A" -StorageSubsystemFriendlyName "Storage Spaces*" -PhysicalDisks $LogandSystemDisks | New-VirtualDisk -FriendlyName "SQLLogsandSystem1Disk01A" -Size 1022GB -ProvisioningType Fixed -ResiliencySettingName Simple|Initialize-Disk -PassThru | New-Partition -DriveLetter 'G' -UseMaximumSize
		...
		Script Configure-MountPoints
        	{
                ...
                $LogandSystemDBPath = 'C:\DataRoot\LogandSystemDB'
		        ...
		        if (!(Test-path $LogandSystemDBPath))
		        {
    		        $logpathfoldercreateresult = New-Item $LogandSystemDBPath -type Directory
                    write-output $logandsystempathfoldercreateresult
		        }
                ...
                if($Logandsystemmpdrive -eq $Null)
                {
                ...
                $logandsystemmpdrive=$gdrive|where-object AccessPaths -eq 'C:\DataRoot\LogandSystemDB\'
                    ...
                    if($datampdrive -ne $Null -and $logandsystemmpdrive -ne $null)
                    {
                        $true
                    }
                    else
    ...
    #C:\SQLServer_12.0_Full\setup.exe /q /Action=Install /IACCEPTSQLSERVERLICENSETERMS /UpdateEnabled=True /UpdateSource=C:\SQLServer_12.0_Full\PCU /FEATURES=SQLEngine,FullText,RS,IS,BC,Conn,ADV_SSMS /ASCOLLATION=Latin1_General_BIN /InstanceName=$InstanceName /SQLBACKUPDIR=C:\DataRoot\SystemDB\Backup /INSTALLSQLDATADIR=C:\DataRoot\SystemDB /SQLSYSADMINACCOUNTS='+$LocalAdminSQL +' /SQLSVCSTARTUPTYPE=AUTOMATIC /SQLTEMPDBDIR=D:\TempDB\MSSQL\Data /SQLTEMPDBLOGDIR=D:\TempDB\MSSQL\Logs /SQLUSERDBDIR=C:\DataRoot\Data1 /SQLUSERDBLOGDIR=C:\DataRoot\Logs /RSINSTALLMODE=FilesOnlyMode
