...
Import-DscResource -ModuleName xActiveDirectory, xComputerManagement, xNetworking,  PSDesiredStateConfiguration, xPendingReboot, xAzureStorage
    Node localhost
    {
        LocalConfigurationManager
        {
...
} #End of sessionhost configuration
...
configuration RDSDeployment
{
   param 
    ( 
        ...
        [String]$sessionHostNamingPrefix = "RDSSH0",
    ...
    { 
        ...
        #$numberofRDSHintances=0
        #$sessionhostnamingprefix='RDSSH0'
        #$domainname='irmchosted.com'
        if($numberofRDSHinstances=1)
        {
            $sessionHosts =@( 1..($numberOfRdshInstances) | % { "$sessionHostNamingPrefix$_.$domainname"} )
            $sessionshosts=$sessionhosts[0]
        }
        else
        {
            $sessionHosts = @( 1..($numberOfRdshInstances) | % { "$sessionHostNamingPrefix$_.$domainname"} )
