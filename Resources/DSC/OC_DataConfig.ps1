...
########################
  ...
  #Download Remote Desktop Connection Manager installer from Microsoft    
    
       Script RDCMInstaller    
        {
         SetScript = 
           {
             ...
             if($Folderpath -eq $Null)
              {
               ...
               #write-output $foldercreateresults
                 $Source = "https://www.microsoft.com/en-us/download/confirmation.aspx?id=44989"
                 $Destination = "C:\installs\rdcman.msi"
         ...
         TestScript = 
           { 
             $RDCMinstalldir = get-item C:\installs\rdcman.msi -ErrorAction SilentlyContinue   
         ...
         Script InstallRDCM
          {
           SetScript = { 
             $FilePath = "C:\installs"
             $script = Start-Process -FilePath "C:\installs\rdcman.msi" -ArgumentList "/q" -wait
                    
                       }
           TestScript = { 
             $RDCMinstalled = Get-WmiObject -Class Win32_Product | where { $_.Name -match “Remote Desktop Connection Manager”}
                 if($RDCMinstalled)
                         {
                            $True
                         }
           ...
           GetScript = {$null}
             # Credential = $DomainCreds
           DependsOn = "[Script]RDCMInstaller"
             }        
           GetScript = {$null}
             # Credential = $DomainCreds
           DependsOn = "[Script]GetSSMSInstaller"
########################
  } #end of node       
}#end of configuration

OC_DataConfig -OutputPath "C:\DSC"
